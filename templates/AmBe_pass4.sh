...
# relative way / scp
FILE_PATH="rat-"$RAT_VERSION"/in_grid/backup/"$RUN"/"$SUBFILE"/"
...
OUTPUT_PATH=$LSTORE_AMBE"local/rat-"$RAT_VERSION"/in_grid/"$RUN"/"$SUBFILE"/"
...
# RAT
RAT_MACRO=${RAT_PATH}"rat-"${RAT_VERSION}"/mac/processing/water/fourth_pass_AmBe_processing_wSelection.mac"
...
#$ -l h=!(wn164|wn174|wn179|wn181|wn182|wn183|wn185|wn187|wn200|wn201|wn202|wn203|wn216)
...
/usr/bin/time --verbose --output=${OUTPUT_PATH}"time_pass4_"${RUN}"_"${SUBFILE}".log" /lstore/sno/stefan/rat/rat-6.15.0/bin/rat -d water-ndecay-v2 -i $RAT_INPUT $RAT_MACRO
